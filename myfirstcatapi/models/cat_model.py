...
from myfirstcatapi import config, dto
from myfirstcatapi.exceptions import DuplicateCatError, EmptyResultsFilter
from myfirstcatapi.models.common import (
    BSONDocument,
...
async def create_cat(new_cat: dto.UnsavedCat, now: datetime) -> dto.Cat:
    ...
    logger.info(f"Successfully created Cat {cat_id} in Myfirstcatapi")
    return dto.Cat(
        id=cat_id,
        **unsaved_cat_as_bson,
    )
