...
def assert_is_test_db() -> None:
    assert myfirstcatapi.config.MONGODB_URL.endswith("_test")
...
def mock_config(monkeypatch: Any) -> None:
    ...
    """
    monkeypatch.setattr("myfirstcatapi.config.ENABLE_FOO", "true")
    monkeypatch.setattr("myfirstcatapi.config.ENABLE_BAR", "false")
...
def prevent_actual_requests(monkeypatch: Any) -> None:
    def mock_request(*args: Any, **kwargs: Any) -> None:
        raise Exception(
            ...
            "See myfirstcatapi.tests.conftest.prevent_actual_requests for more information."
        )
