...
autoformat() {
  ...
  echo '---- format (isort) ----'
  poetry run isort myfirstcatapi tests $ISORT_FLAG
}

check_style() {
  echo '---- style ----'
  poetry run flake8 myfirstcatapi tests
}

check_type() {
  echo '---- type ----'
  poetry run mypy myfirstcatapi tests
}

check_unit() {
  # default to coverage mode
  PYTEST_FLAGS='--cov=myfirstcatapi --ignore=myfirstcatapi --cov-report=term-missing --cov-report=xml'
